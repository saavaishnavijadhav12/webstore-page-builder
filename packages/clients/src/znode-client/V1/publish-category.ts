...
export async function PublishCategory_list(
  ...
  pageIndex: number | undefined,
  pageSize: number | undefined
): Promise<MultifrontTypes.PublishCategoryListResponse> {
  let options_: RequestInit = {
    method: "GET",
    cache: "no-store",
    headers: await getHeaders("GET", String(baseUrl)),
...
function PublishCategory_processList(response: Response): Promise<MultifrontTypes.PublishCategoryListResponse> {
  const status = response.status;
  let _headers: any = {};
  if (response.headers && response.headers.forEach) {
    response.headers.forEach((v: any, k: any) => (_headers[k] = v));
 ...
 * @param assignedIds Already assigned Ids
 * @param expand (optional)
 * @param filter (optional)
 * @param sort (optional)
 * @param pageIndex (optional)
...
export async function PublishCategory_unassignedList(
  assignedIds: string,
  expand: string[] | undefined,
  filter: FilterTuple[] | undefined,
  sort: { [key: string]: string } | undefined,
  ...
  let url_ = baseUrl + "PublishCategory/UnassignedList/{assignedIds}";

  url_ += buildEndpointQueryString({ expand, filter, sort, pageIndex, pageSize });

  if (assignedIds === undefined || assignedIds === null) throw new Error("The parameter 'assignedIds' must be defined.");
  url_ = url_.replace("{assignedIds}", encodeURIComponent("" + assignedIds));
  if (expand === null) throw new Error("The parameter 'expand' cannot be null.");
  if (filter === null) throw new Error("The parameter 'filter' cannot be null.");
  if (sort === null) throw new Error("The parameter 'sort' cannot be null.");
  if (pageIndex === null) throw new Error("The parameter 'pageIndex' cannot be null.");
  ...
  return fetch(url_, options_).then((_response: Response) => {
    return PublishCategory_processUnassignedList(_response);
  });
}

function PublishCategory_processUnassignedList(response: Response): Promise<MultifrontTypes.PublishCategoryListResponse> {
  const status = response.status;
  let _headers: any = {};
  if (response.headers && response.headers.forEach) {
    response.headers.forEach((v: any, k: any) => (_headers[k] = v));
...
export async function PublishCategory_getPublishCategory(
  ...
  pageIndex: number | undefined,
  pageSize: number | undefined
): Promise<MultifrontTypes.PublishCategoryResponse> {
...
function PublishCategory_processGetPublishCategory(response: Response): Promise<MultifrontTypes.PublishCategoryResponse> {
  const status = response.status;
  let _headers: any = {};
  if (response.headers && response.headers.forEach) {
    response.headers.forEach((v: any, k: any) => (_headers[k] = v));
